...
import { isTimeSlotDuringLunchBreak } from '@/lib/lunch-break-utils';
...
export async function GET(request: Request) {
  try {
    ...
    // Get business hours for lunch break information
    ...
    const availableStaff = service.staff.map((staff: any) => {
      ...
      if (isAvailable && time && daySchedule && daySchedule.start && daySchedule.end) {
        ...
        if (isAvailable && time) {
          const serviceEndTimeString = format(serviceEndTime, 'HH:mm');
          
          // Check business lunch break
          if (businessHours?.lunchBreakStart && businessHours?.lunchBreakEnd) {
            const isDuringBusinessLunchBreak = isTimeSlotDuringLunchBreak(
              { start: time, end: serviceEndTimeString },
              { start: businessHours.lunchBreakStart, end: businessHours.lunchBreakEnd },
              bookingDate
            );
            if (isDuringBusinessLunchBreak) {
              isAvailable = false;
            }
          }

          // Check staff individual lunch break
          if (isAvailable && daySchedule?.lunchBreakStart && daySchedule?.lunchBreakEnd) {
            const isDuringStaffLunchBreak = isTimeSlotDuringLunchBreak(
              { start: time, end: serviceEndTimeString },
              { start: daySchedule.lunchBreakStart, end: daySchedule.lunchBreakEnd },
              bookingDate
            );
            if (isDuringStaffLunchBreak) {
              isAvailable = false;
            }
          }
        }
