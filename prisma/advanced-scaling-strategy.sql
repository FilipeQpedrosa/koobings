-- 🚀 ADVANCED DATABASE SCALING STRATEGY
...
-- ===============================================
-- 3. OPTIMIZED INDEXES ON PARTITIONS
-- ===============================================

-- Performance indexes for each partition
DO $$
DECLARE
    partition_name TEXT;
BEGIN
    FOR i IN 0..15 LOOP
        partition_name := 'appointments_p' || i;
        
        -- Core performance indexes
        EXECUTE format('CREATE INDEX CONCURRENTLY IF NOT EXISTS %I_scheduled_for_idx ON %I ("scheduledFor")', partition_name, partition_name);
        EXECUTE format('CREATE INDEX CONCURRENTLY IF NOT EXISTS %I_staff_idx ON %I ("staffId")', partition_name, partition_name);
        EXECUTE format('CREATE INDEX CONCURRENTLY IF NOT EXISTS %I_client_idx ON %I ("clientId")', partition_name, partition_name);
        EXECUTE format('CREATE INDEX CONCURRENTLY IF NOT EXISTS %I_status_idx ON %I (status)', partition_name, partition_name);
        
        -- Composite indexes for common queries
        EXECUTE format('CREATE INDEX CONCURRENTLY IF NOT EXISTS %I_business_date_idx ON %I ("businessId", "scheduledFor")', partition_name, partition_name);
        EXECUTE format('CREATE INDEX CONCURRENTLY IF NOT EXISTS %I_staff_date_idx ON %I ("staffId", "scheduledFor")', partition_name, partition_name);
    END LOOP;
END $$;
...
SELECT 
    ...
    ds.max_businesses,
    ROUND((ds.current_businesses::float / ds.max_businesses * 100), 2) as utilization_percent,
    ds.status
...
-- Success message
SELECT 'Advanced Database Scaling Strategy implemented successfully!' as status; 